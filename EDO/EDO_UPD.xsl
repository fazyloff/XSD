			...
			<Документ КНД="1115106" НаимДокОпр="Счет-фактура и документ об отгрузке товаров (выполнении работ), передаче имущественных прав (документ об оказании услуг)" ПоФактХЖ="Документ об отгрузке товаров (выполнении работ), передаче имущественных прав (документ об оказании услуг)" Функция="СЧФДОП">
				...
				<СвСчФакт>
					<xsl:attribute name="ДатаСчФ">
						<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:DocumentDate"/>
					</xsl:attribute>
					<xsl:attribute name="КодОКВ">
						<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:CurrencyCode"/>
					</xsl:attribute>
					<xsl:attribute name="НомерСчФ">
						<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:InvoiceId"/>
					</xsl:attribute>
					<СвПрод>
						<xsl:attribute name="ОКПО">
							<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:GRD_EdoSupplierOKPO"/>
						</xsl:attribute>
						<ИдСв>
							<СвЮЛУч>
								<xsl:attribute name="ИННЮЛ">
									<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:GRD_EdoSupplierINN"/>
								</xsl:attribute>
								<xsl:attribute name="КПП">
									<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:GRD_EdoSupplierKPP"/>
								</xsl:attribute>
								<xsl:attribute name="НаимОрг">
									<xsl:value-of select="a:CustInvoiceJour_GRD/a:CustInvoiceJour/a:GRD_EdoSupplierName"/>
								</xsl:attribute>
							</СвЮЛУч>
						</ИдСв>
					</СвПрод>
